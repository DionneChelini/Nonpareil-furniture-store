...
export default function product_catelogue({
  data,
  meta,
}: {
  data: any;
      ...
      <main className='pb-24'>
        ...
        >
          ...
          <div className='-mx-px grid grid-cols-2 border-l border-gray-200 sm:mx-0 md:grid-cols-4 lg:grid-cols-4'>
            {data.map((product: any) => (
              <Link
                href={`http://localhost:3000/collection/luxury-branded-watches/${product.id}`}
                key={product.id}
              ...
              >
                <div className='aspect-w-1 aspect-h-1 overflow-hidden rounded-lg bg-gray-200 group-hover:opacity-75'>
                  {product.images.data[0].attributes.formats.large ? (
                    <img
                      src={`http://localhost:1337${product.images.data[0].attributes.formats.large.url}`}
                      alt={product.imageAlt}
                      className='h-full w-full object-cover object-center'
                    />
                  ) : product.images.data[0].attributes.formats.medium ? (
                    <img
                      src={`http://localhost:1337${product.images.data[0].attributes.formats.medium.url}`}
                      alt={product.imageAlt}
                      className='h-full w-full object-cover object-center'
                    />
                  ) : product.images.data[0].attributes.formats.small ? (
                    <img
                      src={`http://localhost:1337${product.images.data[0].attributes.formats.small.url}`}
                      alt={product.imageAlt}
                      className='h-full w-full object-cover object-center'
                    />
                  ) : (
                    <img
                      src={`http://localhost:1337${product.images.data[0].attributes.formats.thumbnail.url}`}
                      alt={product.imageAlt}
                      className='h-full w-full object-cover object-center'
                    />
                  )}
        ...
        >
          <div className='min-w-0 flex-1'>
            <Link
              href={`http://localhost:3000/collection/luxury-watches-by-brand/audemars%20piguet/${
          ...
          <div className='flex min-w-0 flex-1 justify-end'>
            <Link
              href={`http://localhost:3000/collection/luxury-watches-by-brand/audemars%20piguet/${
                Number(router.query.page) < pageArray.length
                  ? Number(router.query.page) + 1
                  : pageArray.length.toString()
              }`}
              className='inline-flex h-10 items-center rounded-md border border-gray-300 bg-white px-4 hover:bg-gray-100 focus:border-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:ring-opacity-25 focus:ring-offset-1 focus:ring-offset-indigo-600'
            >
              Next

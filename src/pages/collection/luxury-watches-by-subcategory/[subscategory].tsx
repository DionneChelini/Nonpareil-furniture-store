...
import { ShoppingBagIcon, UserIcon, XCircleIcon } from '@heroicons/react/24/outline'
...
export default function WithProductGridAndPagination({ products }: { products: any }) {
    ...
    <div className="bg-white">
      ...
      <main className="pb-24">
          ...
          <div className="-mx-px border-l border-gray-200 grid   grid-cols-2 sm:mx-0 md:grid-cols-3 lg:grid-cols-4">
            {products.map((product: any) => (
              <div key={product.id} className="group relative   p-4 border-r border-b border-gray-200 sm:p-6">
                <div className="rounded-lg overflow-hidden m-3 bg-gray-200 aspect-w-1 aspect-h-1 group-hover:opacity-75">
                  <Image
                    src={product.imageSrc}
                    alt={product.imageAlt}
                    className="w-full h-full sm:p-6 p-2 object-center object-cover"
                  />
                </div>
        ...
        <nav
          ...
          className="max-w-7xl mx-auto px-4 mt-6 py-4 flex justify-between text-sm font-medium text-gray-700 sm:px-6 lg:px-8"
        >
...
export async function getStaticPaths() {
  ...
  const paths = products.data.map((item: any) => {
    return {
      params: { products: item.id.toString() }
    }
  })

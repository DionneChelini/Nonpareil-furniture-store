...
export default function Example({ data }: { data: Person[] }) {

  console.log(data[0].attributes.image);
  return (
    <div className=" mt-20 max-w-7xl mx-auto sm:px-8">
      <Head>
        <title>Who We Are - Nonpareil Collection</title>
      </Head>
        ...
        <div className="mx-auto py-12 px-4 max-w-7xl sm:px-6 lg:px-8 lg:py-24">
          <div className="space-y-12 lg:grid lg:grid-cols-3 lg:gap-8 lg:space-y-0">
            ...
            <div className="lg:col-span-2">
              ...
              >
                {data.map((person: any) => (
                  <li key={person.attributes.name} className="sm:py-8">
                    <div className="space-y-4 sm:grid sm:grid-cols-3 sm:items-start sm:gap-6 sm:space-y-0">
                      <div className="aspect-w-3 aspect-h-2 sm:aspect-w-3 sm:aspect-h-4">
                        <Image width={500} height={500} className="object-cover shadow-lg rounded-lg" src={'https://strapi342134.s3.amazonaws.com/small_avatar_ef077e4fe6.png'} alt="" />
                      </div>
                      <div className="sm:col-span-2">
                        <div className="space-y-4">
                          <div className="text-lg leading-6 font-medium space-y-1">
                            <h3>{person.attributes.name}</h3>
                            <p className="text-indigo-600">{person.attributes.role}</p>
                          </div>
...
export const getStaticProps: GetStaticProps = async () => {
  const res = await fetch(`${process.env.NEXT_PUBLIC_URL_STRAPI}/api/about-uses/?populate=*`)
